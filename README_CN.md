...
8. [Linux 快速开始](#linux-快速开始)
9. [Bark 配置](#bark-配置)
10. [Claude Code Hooks](#claude-code-hooks)
11. [通知策略](#通知策略)
12. [通知人格包](#通知人格包)
13. [macOS 菜单栏 App](#macos-菜单栏-app)
14. [Windows 托盘 App](#windows-托盘-app)
15. [常用命令](#常用命令)
16. [测试验证](#测试验证)
17. [常见问题排查](#常见问题排查)
18. [隐私与安全](#隐私与安全)
...
在 Linux 上，AgentWatch 以终端监控面板的形式运行，并且可以通过 `notify-send`
弹出**本机桌面通知**——既可以与推送到 Apple Watch / 手机的 Bark 并存，也可以
单独使用。这样你在电脑前工作时，不用掏手机也能看到提醒。

### 前置条件
- 任意现代 Linux 发行版（GNOME / KDE 等任意桌面环境）
- Python 3.10+
- Claude Code（CLI 或 VS Code 插件）
- `notify-send`（本机桌面通知所需，可选但推荐）：
  - Debian/Ubuntu：`sudo apt install libnotify-bin`
  - Fedora：`sudo dnf install libnotify`
  - Arch：`sudo pacman -S libnotify`
- iPhone / Android 手机装 [Bark](https://apps.apple.com/app/bark/id1403753865)（可选——仅在需要手表/手机推送时）
...
# 配置 Python 环境（自动建 .venv、安装、init、doctor）
bash linux/setup.sh

# 配置 Bark key（可选——本机桌面通知无需 Bark 也能用）
source .venv/bin/activate
agentwatch config bark
agentwatch config test

# 安装 Claude Code hooks（手动一次性，复用跨平台脚本）
bash install_claude_hooks.sh
...
```bash
bash linux/agentwatch-monitor.sh    # 激活 .venv 并运行 `agentwatch start`
```
...
该脚本会把 `agentwatch.desktop` 安装到 `~/.local/share/applications/`，之后即可
从应用程序菜单启动 AgentWatch。删除：`rm ~/.local/share/applications/agentwatch.desktop`。

### 本机桌面通知

`config.json` 中的 `desktop_notify` 段控制本机通知：
...
- `enabled`——开关本机桌面通知（默认 `true`）。
- 它与 Bark **并行**工作：关掉 Bark 就只有桌面通知；两者都配就两者都发。
- 在 macOS / Windows 上，或找不到 `notify-send` 时，该后端会静默跳过——不报错，
  Bark 行为与改造前完全一致。
